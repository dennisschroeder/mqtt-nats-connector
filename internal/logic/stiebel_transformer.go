...
type StiebelTransformer struct{}

func (t *StiebelTransformer) Accepts(topic string) bool {
	return strings.HasPrefix(topic, "stiebel-modbus2mqtt-go/")
}

func (t *StiebelTransformer) Transform(topic string, payload []byte) (string, string, *envelope.EventEnvelope) {
	...
	event := &envelope.EventEnvelope{
		Payload: &envelope.EventEnvelope_BinarySensor{
			BinarySensor: &binary_sensor.BinarySensorEvent{
				EntityId:    deviceID,
				State:       state,
				DeviceClass: "heat_pump",
			},
		},
	}
	return "stiebel-modbus2mqtt-go", deviceID, event
}
